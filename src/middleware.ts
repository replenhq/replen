...
export async function middleware(request: NextRequest) {
  ...
  if (
    ...
    // /api/settings/immersion + /api/settings/autoground: token-authed CLI
    // toggles (the /settings web form uses the session-based settings routes).
    // Scoped to these exact paths so the session-auth settings routes stay protected.
    request.nextUrl.pathname === "/api/settings/immersion" ||
    request.nextUrl.pathname === "/api/settings/autoground" ||
