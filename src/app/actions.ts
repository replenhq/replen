...
export async function createHandoff(matchId: number): Promise<{ ok: boolean; prUrl?: string; reason?: string }> {
  ...
  // Footer attribution: links back to the user's own Replen dashboard so a
  // reviewer can see where this came from. PUBLIC_BASE_URL is the same convention
  // used by the email digest. We deliberately don't hardcode a marketing URL —
  // the user can swap this for replen.dev (or a tenant-specific landing page)
  // later by setting PUBLIC_BASE_URL or editing this line directly.
  const dashboard = process.env.PUBLIC_BASE_URL ?? null;
  const attribution = dashboard
    ? `\n\n---\n_Generated by [Replen](${dashboard}) — proactive OSS discovery for your codebase._`
    : `\n\n---\n_Generated by Replen — proactive OSS discovery for your codebase._`;
...
Merge or close - either way the next handoff goes into its own PR.${attribution}`;
