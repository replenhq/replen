...
// Skill-mode home page. The webapp's job here is calm:
//   - Hero ("What could you make better today?")
//   - Activity timeline: every meaningful event from user_match_state.
//     Star, hide, handoff, in chronological order. The "see Replen
//     working" surface.
//
// What's NOT here (intentionally):
//   - Candidate firehose feed — the agent reads the inventory from the
//     /api/inventory/today endpoint, the user doesn't need to see it.
//   - Refresh button, pipeline log, filter bar — all hosted-tier
//     surfaces. Moved to _legacy/.
//   - "Skill mode" banner — calling out a mode that has no alternative.
...
export default async function Home() {
  ...
  // Pull recent activity: any user_match_state row that has been
  // actioned. Ordered by actionAt DESC. Joins repos + project_profiles
  // inline so we render flat. Demo mode shows the same query against
  // the demo user's seeded state.
  const events = await fetchRecentEvents(user.id, 50);

  // First-name greeting if we can derive one from the email. Falls
  // back to a neutral hello otherwise. Demo gets the brand-neutral
  // copy.
