...
import { createHandoff, refreshHandoffStatuses, setMatchStatus } from "@/app/actions";
import { BulkBar, RowCheck } from "@/app/starred/BulkBar";
        ...
        return (
          <div className="match" key={m.id}>
            <div className="match-head">
              ...
              <span className="meta">
                {project ? <>→ <a href={`/demo/?project=${project.slug}`}>{project.slug}</a> · </> : "_general · "}
              ...
              {bucket === "awaiting" && !project?.githubFullName && (
                <span className="meta" style={{ color: "#a96" }}>
                  {project ? <>(set <code>github_full_name</code> on <a href="/demo/projects">/projects</a>)</> : "_general · no project repo"}
                </span>
              )}
              {m.handoffPrUrl && (
  ...
  if (surfaced.length > 0) {
    const links = surfaced.map((s, i) => {
      ...
      return (
        <span key={s.matchId}>
          {i > 0 ? ", " : ""}
          <a href={`/demo/?project=${slug}#m-${s.matchId}`} style={{ color: "#1d4ed8" }}>{slug}</a>
        </span>
      );
    });
    return (
